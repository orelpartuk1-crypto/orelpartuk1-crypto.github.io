  ...
  return (
    <div className="pb-28">
      <TopBar
        ...
        back
        right={
          <button onClick={() => setShowNew((s) => !s)} className="flex h-10 w-10 items-center justify-center rounded-full bg-brand-500 text-white shadow-sm active:scale-95" aria-label="New date">
      ...
      <div className="mx-auto max-w-md px-4 space-y-4">
        ...
        {showNew && (
          <div className="card space-y-3">
            <div>
              ...
              <input className="field" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g. Adi's birthday" />
            </div>
            ...
            <button className="btn-primary w-full" onClick={create}>Add date</button>
          </div>
        )}
        ...
        {!loading && dates.length === 0 && !showNew && (
          <div className="card py-10 text-center">
            ...
            <p className="mt-2 font-semibold">No dates yet</p>
            <p className="text-muted text-sm">Add birthdays and holidays to plan for them.</p>
            <button className="btn-primary mt-4 inline-flex" onClick={() => setShowNew(true)}>Add a date</button>
          </div>
        )}
          ...
          return (
            <div key={d.id} className="card flex items-center justify-between">
              <div className="min-w-0">
                ...
                <p className="text-sm text-muted">
                  {fmt(d._next)}{d.recurring ? ' · yearly' : ''}{d.budget > 0 ? ` · budget ${money(d.budget)}` : ''}
                </p>
              </div>
              <div className="flex items-center gap-3">
                ...
                <button onClick={() => deleteDate(d.id)} className="text-slate-300 hover:text-red-500" aria-label="Delete">
