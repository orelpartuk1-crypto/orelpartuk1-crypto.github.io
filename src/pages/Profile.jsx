    ...
    <div className="pb-28">
      ...
      <Screen className="mx-auto max-w-md px-4 space-y-4">
        <Item className="card flex flex-col items-center gap-3 py-6">
          ...
          <Tap className="btn-primary w-full" onClick={save}>{saved ? 'Saved ✓' : 'Save'}</Tap>
          <p className="text-xs text-muted">{user?.email}</p>
        </Item>

        <Item className="card">
          <h2 className="label">{household?.name || 'Your household'}</h2>
            ...
            {members.map((m) => (
              <li key={m.id} className="flex items-center gap-3 py-2.5">
                ...
                <span className="min-w-0 flex-1">
                  ...
                  <span className="block text-xs text-muted">{m.id === user?.id ? 'You' : 'Partner'}</span>
                </span>
              </li>
            ))}
          </ul>

          <p className="mt-3 text-sm text-muted">Share this code so someone can join:</p>
          <Tap
            onClick={copyCode}
          ...
          >
            ...
            <span className="font-medium text-brand-600">{copied ? 'Copied!' : 'Copy'}</span>
          </Tap>
        </Item>
          ...
          <Link to="/settings" className="card-tap flex items-center justify-between">
            <span className="flex items-center gap-3">
              ...
              <span>
                ...
                <span className="block text-sm text-muted">Appearance, categories, reminders</span>
              </span>
            </span>
        ...
        <Item>
          <Tap className="btn-ghost w-full text-spend" onClick={signOut}>Sign out</Tap>
        </Item>
      </Screen>
    </div>
  )
        ...
        <Tap className="flex w-full items-center justify-between" onClick={() => setOpen(true)}>
          <span className="flex items-center gap-3">
            ...
            <span className="text-left">
              ...
              <span className="block text-sm text-muted">{msg || 'For this account'}</span>
            </span>
          </span>
          <span className="text-muted">›</span>
        </Tap>
  ...
  return (
    <div className="card space-y-3">
      ...
      <div className="grid grid-cols-2 gap-2">
        <Tap className="btn-ghost py-3 text-base" onClick={() => { setOpen(false); setErr(null) }}>Cancel</Tap>
